...
#if defined(PIOS_INCLUDE_MS4525)

#define SAMPLING_DELAY_MS_MS4525                50     //Update at 20Hz
...
#define CALIBRATION_COUNT_IDLE                  (CALIBRATION_IDLE_MS/SAMPLING_DELAY_MS_MS4525) //Ticks to wait before calibrating
#define CALIBRATION_COUNT                       (CALIBRATION_COUNT_MS/SAMPLING_DELAY_MS_MS4525) //Ticks to wait while calibrating
...
void baro_airspeedGetMS4525(BaroAirspeedData *baroAirspeedData, uint32_t *lastSysTime, uint8_t airspeedSensorType, int8_t airspeedADCPin)
{
	//Wait until our turn.
	PIOS_Thread_Sleep_Until(lastSysTime, SAMPLING_DELAY_MS_MS4525);
	...
	baroAirspeedData->SensorValue = PIOS_MS4525_ReadAirspeed();
